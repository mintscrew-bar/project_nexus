...
export class UserService {
  ...
  async updateProfile(
    ...
    if (data.username !== undefined) {
      ...
      // 무엇을 막을지만 정한다.
      //
      // 전에는 영문·숫자·완성형 한글·밑줄만 받는 허용 목록이었다. 그래서
      // 디스코드 이름을 그대로 쓰는 사람들이 프로필을 저장할 수 없었다 —
      // 공백, 마침표, "ㅡ" 같은 자모, 그 밖의 기호가 전부 막혔다
      // (2026-09-20 운영에서 400 연속 발생). 기호를 하나씩 허용 목록에
      // 더하면 빠지는 게 계속 생기므로, 실제로 문제를 일으키는 것만 막는다.
      //
      // - 제어문자: 줄바꿈·탭이 이름에 들어가면 목록과 채팅이 깨진다.
      // - 보이지 않는 문자(zero-width)와 글자 방향 제어: 같은 이름으로
      //   보이게 만들어 남을 사칭할 수 있다.
      // - 이모지: 줄 높이를 밀어 목록 정렬이 흔들린다.
      if (/[\u0000-\u001F\u007F]/.test(trimmed)) {
        throw new BadRequestException(
          "유저네임에 줄바꿈이나 제어문자는 쓸 수 없습니다.",
        );
      }
      if (/[\u200B-\u200F\u202A-\u202E\u2066-\u2069\uFEFF]/.test(trimmed)) {
        throw new BadRequestException(
          "유저네임에 보이지 않는 문자는 쓸 수 없습니다.",
        );
      }
      if (/\p{Extended_Pictographic}/u.test(trimmed)) {
        throw new BadRequestException("유저네임에 이모지는 쓸 수 없습니다.");
      }
      // 글자나 숫자가 하나도 없는 이름(예: "...", "---")은 사람을 가리키지 못한다.
      if (!/[\p{L}\p{N}]/u.test(trimmed)) {
        throw new BadRequestException(
          "유저네임에는 글자나 숫자가 최소 하나는 들어가야 합니다.",
        );
      }
