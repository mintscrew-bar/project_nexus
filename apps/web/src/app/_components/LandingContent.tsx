...
export function LandingContentSections({ showBanner = true }: { showBanner?: boolean }) {
  return (
    <>
      {/* 히어로 — 배너 캐러셀보다 먼저 핵심 가치와 CTA를 노출한다.
          데스크톱에서는 중앙 정렬로 키워 첫 화면(1440x900)에서 캐러셀과 함께 보이게 한다. */}
      {showBanner && (
        <section className="px-6 pt-8 pb-4 md:pt-14 md:pb-8">
          <div className="mx-auto max-w-3xl md:text-center">
            ...
            <p className="mt-3 text-sm leading-relaxed text-text-secondary md:mt-5 md:text-lg">
              팀 구성, 경매 드래프트, 전적 기록까지 — 디스코드 채팅 없이 한 화면에서.
            </p>
            <div className="mt-5 flex gap-3 md:mt-7 md:justify-center">
              <Link
                href="/auth/login"
                className="rounded-lg bg-accent-primary px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-accent-hover md:px-6 md:py-3 md:text-base"
              >
                무료로 시작하기
              </Link>
              <Link
                href="/tournaments"
                className="rounded-lg border border-bg-tertiary bg-bg-secondary px-4 py-2.5 text-sm font-semibold text-text-secondary transition-colors hover:text-text-primary md:px-6 md:py-3 md:text-base"
              >
                내전방 보기
              </Link>
            </div>
