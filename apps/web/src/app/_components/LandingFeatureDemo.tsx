    ...
    <div className="absolute inset-x-6 bottom-5 rounded-2xl border border-white/[0.08] bg-[#101116]/95 p-4 shadow-2xl shadow-black/30">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-white/80">참가 준비</p>
        </div>
        <p className="text-lg font-bold tabular-nums text-white">
          {count} <span className="text-white/60">/ 10</span>
        </p>
      </div>
      ...
      <div className="mt-4 grid grid-cols-5 gap-2">
        {["TOP", "JGL", "MID", "BOT", "SUP"].map((position, index) => (
          <div key={position} className="text-center">
            <div
              className={`mx-auto flex h-7 w-7 items-center justify-center rounded-full border ${index < 4 || ready ? "border-emerald-300/25 bg-emerald-300/10" : "border-indigo-400/20 bg-indigo-400/[0.08]"}`}
            >
              <Check
                className={`h-3 w-3 ${index < 4 || ready ? "text-emerald-200" : "text-white/35"}`}
              />
            </div>
      ...
      <button
        ...
        className={`mt-4 flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-[10px] font-bold transition ${ready ? "border border-emerald-300/20 bg-emerald-300/10 text-emerald-200" : "bg-indigo-400 text-[#0d0e1a] hover:bg-indigo-300"}`}
      >
        <CircleDot className="h-3 w-3" />
        {ready ? "준비 완료 · 다시 누르면 취소" : "내 준비 상태 변경"}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="absolute inset-x-6 bottom-5 rounded-2xl border border-white/[0.08] bg-[#101116]/95 p-4 shadow-2xl shadow-black/30">
      <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
        <div>
          <p className="text-[9px] font-bold tracking-[0.16em] text-white/60">
            {tab === "result" ? "MATCH RESULT" : "BROADCAST OVERLAY"}
          </p>
          <p className="mt-1 text-xs font-semibold text-white/80">
            금요일 정기 내전 · 3세트
          </p>
      ...
      {tab === "result" ? (
        ...
        <div className="mt-4 rounded-xl border border-violet-300/15 bg-violet-300/[0.05] p-3">
          <div className="flex items-center gap-2 text-[10px] font-bold text-violet-100">
            <Radio className="h-3.5 w-3.5" />
            OBS 브라우저 소스 미리보기
          </div>
          <div className="mt-3 flex items-center justify-between text-[9px] text-white/55">
            <span>대기 → 경매 → 대진표 → 결과</span>
            <span className="text-emerald-200">연결됨</span>
          </div>
        </div>
      )}
      <div className="mt-3 flex gap-2">
        <button
          ...
          className={`rounded-lg px-2.5 py-1.5 text-[9px] font-bold ${tab === "result" ? "bg-white/10 text-white" : "text-white/40"}`}
        >
        ...
        <button
          ...
          className={`rounded-lg px-2.5 py-1.5 text-[9px] font-bold ${tab === "broadcast" ? "bg-white/10 text-white" : "text-white/40"}`}
        >
