...
/**
 * 스네이크 드래프트 방송 보드(순수 뷰) — 픽 순서/현재 픽 팀/타이머/로스터를 그린다.
 * 라이브 데이터는 BroadcastSnakeDraftLive(훅 컨테이너)가 주입하고,
 * 프리뷰 페이지는 목 데이터로 직접 렌더한다. (AuctionBoardView와 같은 위상)
 */
...
export function SnakeDraftBoardView({
  draft,
  snapshot,
}: {
  draft: BroadcastSnakeDraftData;
  snapshot: any;
}) {
