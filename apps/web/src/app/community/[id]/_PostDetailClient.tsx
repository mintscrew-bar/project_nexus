...
export default function PostDetailClient() {
  ...
  type ReportTarget =
    | { type: "post"; id: string }
    | { type: "comment"; id: string; authorName: string };
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      if (reportTarget.type === "post") {
        await communityApi.reportPost(reportTarget.id, {
          reason: reportReason,
          description: reportDescription.trim(),
        });
      } else {
        await communityApi.reportComment(reportTarget.id, {
          reason: reportReason,
          description: reportDescription.trim(),
        });
      }
  ...
  const legacyConfig = post.category
    ...
    <>
      <div className="flex-grow p-4 md:p-8 animate-fade-in">
        <div className="container mx-auto max-w-3xl">
            ...
            <CardContent className="p-4 md:p-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-y-2 pt-6 border-t border-bg-tertiary">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <Bookmark
                      className={`h-4 w-4 mr-1 ${hasBookmarked ? "fill-current" : ""}`}
                    />
          ...
          <AdSlotCard slotKey="article" minHeight={90} className="my-6" />
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              >
                                <ThumbsUp
                                  className={`h-3 w-3 ${commentLikes[comment.id]?.liked ? "fill-current" : ""}`}
                                />
                          ...
                          {comment.isBlinded ? (
                            ...
                            <div className="flex gap-2 mt-1">
                              ...
                              <button
                                onClick={() =>
                                  handleSubmitEditComment(comment.id)
                                }
                              ...
                              <div className="flex-grow">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    >
                                      <ThumbsUp
                                        className={`h-3 w-3 ${commentLikes[reply.id]?.liked ? "fill-current" : ""}`}
                                      />
                                      {(commentLikes[reply.id]?.count || 0) >
                                        0 && (
                                        <span>
                                          {commentLikes[reply.id]?.count}
                                        </span>
                                      )}
                                    </button>
                                ...
                                {reply.isBlinded ? (
                                  ...
                                  <div className="flex gap-2 mt-1">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleSubmitEditComment(reply.id)
                                      }
                                      className="text-accent-primary hover:text-accent-primary/80"
                                    >
                                      <Check className="h-4 w-4" />
                ...
                {reportTarget.type === "comment" && (
                  <p className="text-xs text-text-tertiary bg-bg-tertiary rounded-lg px-3 py-2">
                    <span className="text-text-secondary font-medium">
                      {(reportTarget as any).authorName}
                    </span>
                    님의 댓글을 신고합니다.
                  </p>
                )}
