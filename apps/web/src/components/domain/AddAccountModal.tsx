...
export function AddAccountModal({
  ...
  const canSubmit =
    mainRoleSatisfied && subRoleSatisfied && mainRole !== subRole;
  ...
  const handleClose = () => {
    if (step === 3) {
      if (!showCloseWarning) {
        ...
        setLocalError(
          "역할 및 챔피언 선택을 완료하지 않으면 내전에 참여할 수 없습니다. 다시 열면 이 단계부터 이어서 진행할 수 있습니다.",
        );
        return;
      }
      setShowCloseWarning(false);
  ...
  const handleRoleSubmit = async () => {
    ...
    if (!mainRoleSatisfied || !subRoleSatisfied) {
      ...
      if (!mainRoleSatisfied)
        missing.push(
          `주 역할(${mainRole}): ${championsByRole[mainRole].length}/3`,
        );
      if (!subRoleSatisfied)
        missing.push(
          `부 역할(${subRole}): ${championsByRole[subRole].length}/3`,
        );
      setLocalError(
        `선호 챔피언을 역할별 최소 3개 선택해주세요. ${missing.join(", ")}`,
      );
      return;
    }
    if (!pendingRiotId) {
  ...
  const handleChampionSelectionChange = (
    role: Role,
    selectedKeys: string[],
  ) => {
  ...
  const modalTitle = () => {
    switch (step) {
      case 1:
        return "Riot 계정 추가 (1/3)";
      case 2:
        return "최고 티어 입력 (2/3)";
      case 3:
        return "역할 및 챔피언 선택 (3/3)";
      default:
        return "Riot 계정 추가";
    }
  };
  ...
  const modalDescription = () => {
    switch (step) {
      case 1:
        return "소환사 이름과 태그라인을 입력하여 Riot 계정을 연동합니다.";
      case 2:
        return "과거 시즌에 달성한 최고 티어가 있으면 입력해주세요.";
      case 3:
        return "주로 플레이하는 역할과 선호 챔피언을 선택해주세요.";
      default:
        return "";
    }
  };
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title={modalTitle()} size="lg">
      {localError && (
        <div className="mb-4 p-3 bg-accent-danger/10 border border-accent-danger rounded-lg flex items-center gap-2">
          ...
          <p className="text-sm text-accent-danger whitespace-pre-line">
            {localError}
          </p>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-bg-tertiary/70 px-3 py-2 text-xs leading-5 text-text-secondary">
            주 역할과 부 역할은 각각 선호 챔피언을 최소 3개 선택해야 합니다.
            최근에 자주 쓰는 챔피언 위주로 골라도 됩니다.
          </div>
          ...
          <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
            ...
            <RoleAccordionSection
              ...
              onSelectionChange={(keys) =>
                handleChampionSelectionChange(mainRole, keys)
              }
            />
            {/* 주 역할 완료 버튼 */}
            ...
            {mainRole !== subRole && (
              <RoleAccordionSection
                ...
                onSelectionChange={(keys) =>
                  handleChampionSelectionChange(subRole, keys)
                }
              />
            )}
            {/* 부 역할 완료 버튼 */}
            ...
            {/* 나머지 역할 — 선택 사항, 접힌 상태 */}
            {ROLES.filter((r) => r !== mainRole && r !== subRole).map(
              (role) => (
                <RoleAccordionSection
                  key={role}
                  role={role}
                  ...
                  onSelectionChange={(keys) =>
                    handleChampionSelectionChange(role, keys)
                  }
                />
              ),
            )}
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <Button
          variant="outline"
          onClick={step === 3 ? handleClose : onClose}
          disabled={isLoading}
        >
        ...
        {step === 1 && (
          <Button
            onClick={handleSummonerSubmit}
            disabled={!gameName.trim() || !tagLine.trim() || isLoading}
          >
  ...
  return (
    <div
      className={`rounded-xl border transition-colors ${
        isExpanded
          ? "border-bg-elevated bg-bg-secondary/50"
          : "border-bg-tertiary"
      }`}
    >
      {/* 아코디언 헤더 */}
