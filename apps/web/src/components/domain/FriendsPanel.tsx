...
function FriendItem({
  ...
  const unread = unreadCounts[friendUser.id] ?? 0;
...
export function FriendsPanel() {
  ...
  useEffect(() => {
    ...
    const handleUnreadCount = (data: { total: number }) => {
    ...
    dmSocketHelpers.onNewMessage(handleNewDm);
    dmSocketHelpers.onUnreadCount(handleUnreadCount);
    dmSocketHelpers.onUserTyping(handleTyping);
  ...
  return (
    <>
      ...
      {/* Slide-in Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full sm:w-72 bg-bg-secondary border-l border-bg-tertiary z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${
