...
interface Participant {
  ...
  /** 자동 밸런스가 배정한 라인 (편성 후에만 존재) */
  assignedRole?: string | null;
  /** 라인별 밸런스 점수 — 편성 확인 화면과 참가자 카드에서 쓴다 */
  balanceScores?: Record<string, number> | null;
  balanceScore?: number | null;
  balanceScoreRole?: string | null;
  ...
  connect: (roomId, password?) => {
    ...
    socket.on(
      ...
      (data: {
        ...
        if (
          currentRoom &&
          !currentRoom.participants.some((p) => p.userId === data.userId)
        ) {
    ...
    socket.on(
      ...
      (data: { userId: string; newRole: "PLAYER" | "SPECTATOR" }) => {
        const currentRoom = get().room;
        if (currentRoom) {
          const updatedParticipants = currentRoom.participants.map((p) =>
            p.userId === data.userId
              ? { ...p, role: data.newRole, isReady: false }
              : p,
          );
          set({ room: { ...currentRoom, participants: updatedParticipants } });
        }
    ...
    socket.on(
      ...
      (data: { userId: string; teamId: string | null }) => {
        const currentRoom = get().room;
        if (currentRoom) {
          const updatedParticipants = currentRoom.participants.map((p) =>
            p.userId === data.userId
              ? { ...p, teamId: data.teamId, isReady: false }
              : p,
          );
          set({ room: { ...currentRoom, participants: updatedParticipants } });
        }
    ...
    socket.on(
      ...
      (data: { userId: string; inVoice: boolean }) => {
        const currentRoom = get().room;
        if (currentRoom) {
          const updatedParticipants = currentRoom.participants.map((p) =>
