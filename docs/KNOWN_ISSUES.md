...
**문제**:
- 전적 목록(접힌 상태) 카드에서 모바일 화면에 아이템, CS, 딜량 등 핵심 정보가 표시되지 않음
- 팀 상세 탭은 2줄 레이아웃 적용 완료되었으나, 접힌 매치 카드 자체의 모바일 정보량이 부족

**위치**:
- `apps/web/src/components/matches/RiotMatchList.tsx` — 접힌 매치 카드 영역 (약 700~800행 부근)

**해결 방법**:
- [ ] 접힌 매치 카드에 모바일용 2행 레이아웃 적용 (아이템 + CS + 딜량 표시)
- [ ] 현재 `hidden md:flex`로 숨겨진 정보를 모바일에서도 표시
