...
- [x] Task 10: 낙찰(player-sold) 피드백
  - 낙찰 정보 토스트 ("팀A가 유저B를 500G에 낙찰!")
  - 매물 전환 시 fade-in 애니메이션

- [x] Task 11: 유찰(player-unsold) 피드백
  - 유찰 시 선수 카드 shake 애니메이션

- [x] Task 12: 입찰 실시간 피드백
  - 타인 입찰 시 최고입찰가 영역 flash 효과
  - 매물 전환 시 fade 애니메이션
...
- [x] Task 13: 비캡틴/관전자 관전 UX
  - 일반 선수에게 "관전 모드" 안내 문구 표시 (입찰 패널이 안 보이는 이유 설명)
  - 캡틴이지만 다른 팀 차례일 때 "다른 팀의 입찰 차례" 안내 표시

- [x] Task 14: 에러 복구 및 상태 전환
  - 연결 에러 시 "다시 시도" + "로비로 돌아가기" 버튼 추가
  - 팀장 선정 → 경매 시작 사이 전환 화면 개선 (연결 끊김 시 재연결 버튼)

---

## Phase 6: 경매 결과 및 정보 보강

- [x] Task 15: 경매 완료 결과 요약 화면
  - 즉시 리디렉트 대신 5초 카운트다운 + 결과 요약 표시
  - 각 팀 구성원 + 남은 예산 요약
  - "역할 선택으로 이동" 즉시 이동 버튼 + 자동 전환

- [x] Task 16: 입찰 로그 개선
  - 선수별 구분선/라벨 추가 (bid-resolved 시 다음 선수 마커 삽입)
  - 선수 전환 시 로그 구분 명확화
