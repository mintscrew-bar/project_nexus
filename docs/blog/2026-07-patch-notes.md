...
방송 설정 화면에 OBS용 오버레이 기능이 추가되었습니다.

방송 링크를 OBS의 브라우저 소스로 등록하면, 방을 만들 때마다 해당 방의 진행 화면이 자동으로 방송에 출력됩니다. 링크를 매번 새로 바꿀 필요 없이 한 번 등록해두고 계속 사용할 수 있습니다.

![방송 오버레이 설정](./images/2026-07-patch-notes/broadcast-settings.png)

설정 화면에서는 OBS 등록 방법도 함께 안내합니다. `1920 x 1080` 기준으로 브라우저 소스를 추가하면 대기 화면, 경매, 역할 선택, 대진표, 경기, 결과 화면이 방송용 화면으로 표시됩니다.

외부 조작 토큰도 함께 제공됩니다. Stream Deck, Ulanzi 브릿지, 자동화 스크립트 같은 도구에서 방송 장면을 바꾸고 싶을 때 사용할 수 있습니다.

## 방송 조작 패널

방송 화면을 별도로 제어할 수 있는 방송 조작 패널이 추가되었습니다.

OBS에는 컨트롤 UI를 넣지 않고, 조작 패널은 별도 창으로 띄워서 사용할 수 있습니다. 진행자는 패널에서 장면을 바꾸고, OBS에는 시청자에게 보여줄 화면만 출력됩니다.
...
하단 정보바 표시 여부와 짧은 공지 문구도 조작할 수 있습니다. 방송 중 안내 멘트나 잠깐의 공지를 화면에 띄우기 쉬워졌습니다.

## OBS 출력 화면

OBS에서는 실제 방송용 화면만 깔끔하게 표시됩니다.

아래 화면처럼 브라우저 소스를 추가하면 NEXUS 대기 화면이 바로 출력됩니다. 컨트롤러나 관리자 화면은 보이지 않고, 방송에 필요한 장면만 전체 화면으로 잡힙니다.

![OBS 대기 화면](./images/2026-07-patch-notes/obs-standby-overlay.png)

이 구조 덕분에 진행자는 NEXUS 화면을 그대로 캡처하지 않아도 됩니다. 방송용 링크 하나만 OBS에 등록해두면, 내전 진행 흐름에 맞춰 정리된 화면을 보여줄 수 있습니다.
