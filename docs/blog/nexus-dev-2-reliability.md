...
이번 글은 기능 이야기가 아니라 **운영 이야기**입니다. NEXUS는 제가 집에 둔 PC(WSL2 환경) 한 대에서 돌아갑니다. 규모가 크지 않으니 클라우드를 크게 쓰지 않고, 대신 이 한 대가 죽으면 서비스 전체가 죽습니다.

그리고 실제로, 얼마 전 이 서버가 **혼자 여러 번 껐다 켜졌습니다.**
...
Docker 컨테이너에는 `restart: unless-stopped` 정책이 걸려 있습니다. 하지만 이건 "Docker 데몬이 떠 있을 때 컨테이너가 죽으면 되살린다"는 의미고, **머신이 통째로 재부팅되는 상황**은 별개입니다. 부팅 후 compose 스택을 누군가 올려줘야 합니다.

그래서 systemd 유닛으로 부팅 복구를 못박았습니다.
...
## 4. 메모리가 부족해도 핵심 서비스가 살아남게 (swap)

"swap을 추가하자"고 접근했는데, 확인해보니 **WSL2는 이미 swap을 제공하고 있었습니다.**
