...
이제 `pnpm bump`만 치면 `origin/main..HEAD`의 커밋을 분석해 세 `package.json`을 알아서 올립니다.

```bash
$ pnpm bump --dry        # 계산만, 파일은 안 건드림
버전 범프: 1.9.0 → 1.9.1  (patch)
판정 근거:
  유저 체감 feat 없음 → patch

$ pnpm bump              # 실제 적용
$ pnpm bump major        # 마일스톤에 등급 강제
$ pnpm bump patch        # 등급 강제
```

하지만 이건 여전히 "사람이 명령어를 친다"는 전제가 남아 있습니다. 진짜 목표는 **그것마저 없애는 것**이었습니다.
