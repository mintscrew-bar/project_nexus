...
- [ ] Task 3: 알림 end-to-end 점검
  ...
  - 알림 드로어/페이지 완성도 확인
  - 2026-09-22 진행: 헤더에 종(NotificationBell)을 다시 달았다. 친구·클랜 관련은
    알림에서 빼고 친구창 "대기" 탭(받은 친구 요청·클랜 초대·클랜 가입 요청)으로 옮겼다.
    기존 알림은 마이그레이션으로 전부 읽음 처리. 경기 결과 링크를 대진표로 고쳤다.
    남은 것: 실브라우저에서 실시간 수신·읽음·클릭 이동 확인
