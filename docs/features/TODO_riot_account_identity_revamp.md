...
퍼스널 키로 어렵게 모은 매치 데이터는 자산이므로 **버리는 게 아니라 압축해서 들고 간다.**
3계층 구조는 그대로 두되, 2계층(원본)의 수명만 끊는다.

| 계층 | 데이터 | 보존 |
|---|---|---|
| 1. 메모리 | Timeline | LRU only |
| 2. 원본 JSON | `RiotMatchCache` (48KB/행) | **TTL 14일** — 정형화 끝난 것만 삭제 |
| 3. 정형 | `Match` + `MatchParticipant` (~1.5KB/행) | **영구** |

- 2계층을 버려도 되게 만들려면 3계층이 원본의 가치를 흡수해야 한다. 그래서
  `challenges` 129키 중 분석 가치 있는 27개를 컬럼으로 내렸다(커밋 f91febcb).
  Riot 이 서버에서 계산해 주는 값이라 우리 컬럼으로 되계산할 수 없고,
  match-v5 가 오래된 매치를 영구히 주지도 않아 원본이 있는 동안만 이관 가능했다.
- **여기서 얻는 것**: 매치당 약 48KB → 약 15KB(참가자 10행). 원본 4.8GB 가
  정형 데이터 안에 녹아든다.
- 추출 규칙은 `extractChallengeMetrics()` 한 곳에만 둔다. 인제스트와 백필이
  갈리면 신규분과 과거분 통계가 어긋난다.
- 새 지표를 더 뽑고 싶어지면 **원본이 아직 TTL 안에 있는 동안** 해야 한다.
  14일이 지나면 그 매치에서는 영영 못 뽑는다.
