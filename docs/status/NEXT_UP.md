...
- [ ] **킬내기 자동 확정** — 제한시간이 지나면 그 시점 누적 점수로 확정
      ([TODO_pubg_expansion](../features/TODO_pubg_expansion.md) Task 53 잔여).
      지금은 호스트가 직접 확정을 눌러야 한다. 수집은 종료 +45분에 멈추고
      그 사이엔 10분 주기로만 돌아 예산이 새지는 않는다(2026-09-10).
      확정은 방을 COMPLETED 로 바꾸고
      디스코드 공지까지 나가는 **되돌릴 수 없는 동작**이라, 자동으로 돌릴지와
      종료 뒤 얼마를 기다릴지부터 정해야 한다.
